...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      //
      // Legacy compatibility: before_agent_start and resolve_model hooks are
      // also checked for override fields. New hook takes precedence.
      ...
      // Legacy resolve_model hook support (our custom patch)
      if (hookRunner?.hasHooks("resolve_model")) {
        try {
          const resolveResult = await hookRunner.runResolveModel(
          ...
          if (resolveResult?.provider && !modelResolveOverride?.providerOverride) {
            modelResolveOverride = {
              ...modelResolveOverride,
              providerOverride: resolveResult.provider,
            };
          }
          if (resolveResult?.model && !modelResolveOverride?.modelOverride) {
            modelResolveOverride = {
              ...modelResolveOverride,
              modelOverride: resolveResult.model,
            };
          }
        } catch (err) {
